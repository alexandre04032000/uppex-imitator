...
import uppex.semantics.GenModel.{AnnotationBl, Block, Content}
...
object Imitator:
  ...
  //case class AnnotationBl(val name:String,val oldLines:Lines,val newLines:Lines) extends Block:
    //def prettyName: String = s"@$name"
      //case XmlElm(n,_,_) => s"<$n>"
  
  //case class XmlElm(n:String, oldL:Lines, newL:Lines)       extends NamedBl(n,oldL,newL)

  //case class AnnotationBl(n:String, oldL:Lines, newL:Lines) extends ConcreteAnnotationBl(n,oldL,newL)

  /**
   * A Imitator model is seen as a sequence of either
  ...
  case class Model(bs:List[Block]) extends GenModel(bs):
    protected def build(b: Block, isNew: Boolean): String = b match
      case Content(c) => c.toString
      case AnnotationBl(n, _, newTxt) if isNew => s"(*@${n}*)\n${newTxt.mkString("\n")}"
      case AnnotationBl(n, oldTxt, _) => s"(*@${n}*)\n${oldTxt.mkString("\n")}"

    def buildOld: String =
      (for b <- this.bs yield build(b, false)).mkString("\n")
      
    def buildNew: String =
      (for b <- this.bs yield build(b, true)).mkString("\n")
  ...
  def getDiff(m:Model): List[AnnotationBl] =
    for
      (nb:AnnotationBl) <- m.blocks if flat(nb.oldLines) != flat(nb.newLines)
    yield nb
...
//      .mkString("\n")
  ...
  def getPrettyDiff(nb:AnnotationBl): (String,String) =
...
//        "| (...)" :: s"| $hd" :: getPretty(tl,i)
 ...
 /*
  ...
  /** Builds the text of a new Imitator file, using the updated version. */
  def buildNew(m:Model): String =
    (for b<-m.blocks yield build(b,true)).mkString("\n")

  /** Builds the text of a new Imitator file, using the original version. */
...
//    val model = ImitatorParser.parse(fileStr)
