...
import uppex.semantics.GenModel.{Block, NamedBl}
import uppex.semantics.Uppaal.Model

import scala.io.Source

trait GenModel(val blocks: List[Block]):
...
object GenModel:
  ...
  def getDiff(m: GenModel): List[NamedBl] =
    for
  ...
  def getPrettyDiff(m: GenModel): String =
    getDiff(m).map(nb =>
